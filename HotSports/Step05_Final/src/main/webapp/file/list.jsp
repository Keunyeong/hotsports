   ...
   */
   ...
   if(!keyword.equals("")){
      //검색 조건이 무엇이냐에 따라 분기 하기
	   if(condition.equals("title_saveFileName")){//제목 + 내용 검색인 경우
	         //검색 키워드를 FileDto 에 담아서 전달한다.
	         dto.setTitle(keyword);
	         dto.setSaveFileName(keyword);
	         //제목+내용 검색일때 호출하는 메소드를 이용해서 목록 얻어오기 
	         list=FileDao.getInstance().getListTS(dto);
	         //제목+내용 검색일때 호출하는 메소드를 이용해서 row  의 갯수 얻어오기
	         totalRow=FileDao.getInstance().getCountTS(dto);
   ...
   <table>
      ...
      <%for(FileDto tmp:list){%>
         <tr>
            ...
            <td><a href="download.jsp?num=<%=tmp.getNum()%>">다운로드</a></td>
  ...
  <div class="page-ui clearfix">
   ...
   <form action="list.jsp" method="get"> 
      ...
      <select name="condition" id="condition">
      	 <option value="title_saveFileName" <%=condition.equals("title_saveFileName") ? "selected" : ""%>>제목+파일이름</option>
